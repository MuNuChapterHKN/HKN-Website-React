      ...
      {latestEvent && (
        <div className={styles.latestEventContainer} id="latestEvent">
          <Image
            ...
            src={latestEvent.image || '/Common/hkn_ideogramma_white.svg'}
            alt={t('activities.events.latest.poster_alt', { title: latestEvent.title })}
            width="0"
            height="0"
            sizes="100vw"
          />
          <div className={styles.latestEventRight}>
            <span className={styles.latestEventDate}>{latestEvent.date}</span>
            <span className={styles.latestEventTitle}>{latestEvent.title}</span>
            {latestEvent.description && (
              <span className={styles.latestEventDesc}>{latestEvent.description}</span>
            )}
            {latestEvent.link && new Date() < new Date(latestEvent.date) ? (
              latestEvent.booking_link && (
                <RoundButton
                  className={styles.latestEventButton}
                  onClick={() => latestEvent.booking_link && router.push(latestEvent.booking_link)}
                >
              ...
              <RoundButton
                className={styles.latestEventButton}
                onClick={() => latestEvent.link && router.push(latestEvent.link)}
              >
      ...
      {pastEvents.length > 0 && (
        <div className={styles.pastContainer}>
          <span className={styles.pastText}>
            <T keyName="activities.events.past.kicker" />
          </span>
          <span className={styles.pastTitle}>
            <T keyName="activities.events.past.title_line1" />
          </span>
          <span className={styles.pastTitle}>
            <T keyName="activities.events.past.title_line2" />
          </span>
          <div className={styles.pastEvents}>
            ...
            {pastEventsIndex > 0 && (
              <ArrowButton
                left
                className={styles.pastEventsButton_1}
                color="#F2F2F2"
                onClick={() => changeEventsPage(-1)}
              />
            )}
            {pastEventsIndex < pastEvents.length - 1 && (
              <ArrowButton
                right
                className={styles.pastEventsButton_2}
                color="#F2F2F2"
                onClick={() => changeEventsPage(+1)}
              />
            )}
