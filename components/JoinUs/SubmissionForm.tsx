...
export default function SubmissionForm() {
  ...
  const setFileName = (setFileNameFunction: Function, files: FileList | null) => {
    if (files) setFileNameFunction(files[0].name);
  };

  const validateForm = () => {
    let errors: { [key: string]: string } = {};

    if (!acceptGDPR) errors.acceptGDPR = 'Please, accept GDPR terms';

    if (!name) errors.name = 'Please, provide your name';

    if (!surname) errors.surname = 'Please, provide your surname';

    if (!italianLevel) errors.italianLevel = 'Please, select your Italian level';

    if (!average) errors.average = 'Provide your average';
    else if (average < 25.6 && degree === 'Bachelor')
      errors.average = 'Your average must be ≥ 25.6';
    else if (average < 26.6 && degree === 'Master') errors.average = 'Your average must be ≥ 26.6';
    ...
    if (!cv) errors.cv = 'Upload your resume (CV)';
    else if (cv.type !== 'application/pdf') errors.cv = 'Upload a .pdf file';
    else if (cv.size > 2097152) errors.cv = 'File is too large, maximum size: 2MB';

    const studyPlan = (document.getElementById('studyPlan') as HTMLInputElement).files?.[0];
    if (!studyPlan) errors.studyPlan = 'Upload your study plan';
    else if (studyPlan.type !== 'application/pdf') errors.studyPlan = 'Upload a .pdf file';
    else if (studyPlan.size > 2097152) errors.studyPlan = 'File is too large, maximum size: 2MB';
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const studyPlan = (document.getElementById('studyPlan') as HTMLInputElement)
        .files?.[0] as File;
      ...
      if (response.ok) {
        ...
        switch (response.status) {
          case 413:
            setSubmissionError(
              'The files you provided were too large (Max 2MB) or not in the correct format. Try again.'
            );
            break;
          case 422:
  ...
  return submitted ? (
    <section>
      {featureFlags[Feature.ShowImageAfterApply] && (
        <img
          src="/JoinUs/successfulSubmission.png"
          alt={'Submission sent!'}
          width={'100%'}
          height={'auto'}
    ...
    <section>
      ...
      <form className={`${styles.submissionForm} ${styles.gradientBackground}`} onSubmit={onSubmit}>
        <div className={`${styles.halfWidth} ${styles.paddingRight}`}>
          ...
          <input
            ...
            value={name}
            maxLength={100}
            onChange={(e) => updateField(e, 'name', setName)}
            required={true}
          />
        ...
        <div className={`${styles.halfWidth} ${styles.paddingLeft}`}>
          ...
          <input
            ...
            value={surname}
            maxLength={100}
            onChange={(e) => updateField(e, 'surname', setSurname)}
            required={true}
          />
        ...
        <div className={`${styles.halfWidth} ${styles.paddingRight}`}>
          <label htmlFor="average" className={styles.formLabel}>
            Weighted Average
          </label>
          <input
            type="number"
        ...
        <div className={`${styles.halfWidth} ${styles.paddingLeft}`}>
          <label htmlFor="degree" className={styles.formLabel}>
            Type of Degree
          </label>
          <select
            name="degree"
        ...
        <input
          ...
          value={email}
          maxLength={255}
          onChange={(e) => updateField(e, 'email', setEmail)}
          required={true}
        />
        ...
        <label htmlFor="italianLevel" className={styles.formLabel}>
        ...
        <div className={styles.fileInputContainer}>
          ...
          />
          <label htmlFor="cv" className={styles.fileButton}>
            ATTACH A FILE
          </label>
        </div>
        ...
        <div className={styles.fileInputContainer}>
          ...
          />
          <label htmlFor="studyPlan" className={styles.fileButton}>
            ATTACH A FILE
          </label>
        </div>
        ...
        <div className={styles.checkboxContainer}>
          ...
          <label htmlFor="acceptGDPR" className={styles.checkboxLabel}>
            I authorize the treatment of personal data contained in the documents according to the
            D.Lgs. 2018/101 and GDPR (EU Regulation 2016/679).
          </label>
        </div>
      ...
      {submissionError && (
        <span onClick={() => setSubmissionError(null)} className={styles.submissionError}>
          {submissionError}
        </span>
      )}
    </section>
