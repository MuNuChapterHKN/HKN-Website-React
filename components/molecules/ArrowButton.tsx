...
const ArrowButton = ({right, left, className, onClick, style, size, color}:
    ...
    return (
        <div style={{...style, width: circleSize, height: circleSize}} className={`arrowButton ${className}`}
             onClick={onClick}>
            {left ?
                <svg xmlns="http://www.w3.org/2000/svg" className="ionicon" viewBox="0 0 512 512" width={arrowSize}
                     height={arrowSize}>
                    <path fill="#fff" stroke={arrowColor} d="M328 112L184 256l144 144"/>
                </svg>
                :
                <svg xmlns="http://www.w3.org/2000/svg" className="ionicon" viewBox="0 0 512 512" width={arrowSize}
                     height={arrowSize}>
                    <path fill="#fff" stroke={arrowColor} d="M184 112l144 144-144 144"/>
                </svg>
            }
        </div>
    );
